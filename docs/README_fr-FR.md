# Ray Tracing : Démonstration RT accélérée
...
![Static Badge](https://img.shields.io/badge/Projet-Xmake-brightgreen?logo=data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdib3g9IjAgMCA2NCA2NCI+PHBvbHlnb24gcG9pbnRzPSI2NCw0IDY0LDYyIDYyLDY0IDEsNjQgMCw2MyAwLDQwIDYwLDMiIGZpbGw9IiNmZmYiLz48L3N2Zz4=)
<br/>
...
| Intersection rayon-sphère | Boucle bruteforce | Fonction `__device__` |
| Trace de chemins (5 rebonds) | BRDF GGX Microfacet | BRDF GGX Microfacet |
...
| [ISPC](https://ispc.github.io/) | Optionnel | Téléchargement automatique par `Setup.bat` → `vendor/ispc/` |
| [.NET SDK](https://dotnet.microsoft.com/) | Optionnel | Requis uniquement pour la migration `.sln` → `.slnx` |
| [xmake](https://xmake.io/) | Automatique | `Setup.bat` utilise xmake ; installer globalement pour l'utilisation en ligne de commande |
...
- `vsxmake2026/RayTracing.slnx` — ouvrir ceci dans Visual Studio
...
```bash
# Générer la solution VS (après la compilation initiale)
...
│   ├── PathTracer.ispc         # Kernel de traçage de chemins SIMD ISPC
...
│   └── OptiXDenoiser.h/cpp     # Intégration du débruiteur IA OptiX
├── xmake.lua                   # Configuration de compilation (détection CUDA + ISPC)
├── scripts/
│   └── Setup.bat               # Compilation + génération de solution en un clic
...
| `.vcxproj` introuvable dans VS | `vsxmake2026\RayTracing.slnx` obsolète | Relancer `scripts\Setup.bat` ou `xmake project -k vsxmake` puis `dotnet sln vsxmake2026\RayTracing.sln migrate` |
| Le viewport est noir | Incompatibilité d'architecture CUDA | Vérifier que le GPU prend en charge `sm_XX` dans `xmake.lua` |
...
| ISPC introuvable (pas de SIMD) | ISPC absent de `vendor/ispc/` | `Setup.bat` le télécharge automatiquement ; relancer si nécessaire |
| `dotnet sln migrate` échoue | .NET SDK non installé | Installer [.NET SDK](https://dotnet.microsoft.com/) ou ouvrir directement `vsxmake2026\RayTracing.sln` |
