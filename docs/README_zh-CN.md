...
基于 [Walnut](https://github.com/TheCherno/Walnut) 应用框架，使用 C++23 构建的实时交互式路径追踪器。**通过 NVIDIA CUDA 实现 GPU 加速**，**通过 Intel ISPC 实现 CPU 加速**——当 CUDA 可用时，整个路径追踪管线在 GPU 上运行；否则通过 ISPC (AVX2/AVX-512) 使用 CPU SIMD 回退方案。
...
| 未设置 `CUDA_PATH` | 环境变量缺失 | 系统 → 环境变量 → `CUDA_PATH` → CUDA 目录 |
...
| `invalid value 'C++23'` | Walnut 捆绑的旧版 premake5 | 重新运行 `scripts\Setup.bat` 获取 premake5 5.0.0-beta8 |
