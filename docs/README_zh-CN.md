...
一个使用 C++20 和 Walnut 应用框架构建的实时交互式路径追踪器。**通过 NVIDIA CUDA 进行 GPU 加速** — 整个路径追踪管线（光线生成、相交、着色、累积）在 GPU 上运行。当 CUDA 不可用时，回退到 CPU 多线程渲染。
...
| 显示 | Walnut::Image（Vulkan） | Walnut::Image（Vulkan）通过 D2H 复制 |
| 俄罗斯轮盘赌 | CPU | GPU |

**GPU 内核布局**：16×16 线程块，每个像素对应一个 CUDA 线程。`RenderKernel` 为每个像素执行完整的路径追踪，包括光线-球体相交、俄罗斯轮盘赌终止、Lambertian 漫反射 BRDF 采样和渐进式累积。
...
Setup.bat
```
这将运行 Premake5 生成 Visual Studio 2026 解决方案文件。构建系统会自动检测 CUDA 并启用 GPU 加速。

### 4. 构建并运行
在 Visual Studio 2026 中打开 `RayTracing.slnx` 并构建（建议使用 Release 或 Dist 模式以获得性能）。

### 无 CUDA 构建
如果未安装 CUDA Toolkit，项目将构建为仅使用 CPU 的路径追踪器，采用 `std::execution::par`。构建系统仅在检测到 CUDA 时定义 `WL_CUDA`。
...
- **Windows Server 2022**，包含 CUDA 12.8 + Vulkan SDK
- Debug 和 Release 配置
- Release 构建可下载构建产物
...
- 描述\
这是一个简单的 Walnut 应用模板——与 Walnut 仓库中的示例不同，它将 Walnut 作为外部子模块保留，更适合实际构建应用程序。更多详情请参阅 Walnut 仓库。
- 入门指南\
克隆后，您可以根据需要自定义 `premake5.lua` 和 `WalnutApp/premake5.lua` 文件（例如，将名称从 "WalnutApp" 改为其他内容）。满意后，运行 `scripts/Setup.bat` 生成 Visual Studio 2022 解决方案/项目文件。您的应用位于 `WalnutApp/` 目录下，其中 `WalnutApp/src/WalnutApp.cpp` 包含一些基本示例代码以帮助您入门。我建议修改该 WalnutApp 项目来创建您自己的应用程序，因为所有内容都已设置好并准备就绪。
