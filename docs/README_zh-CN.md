...
一个基于 C++20 和 Walnut 应用框架构建的实时交互式路径追踪器。**通过 NVIDIA CUDA 进行 GPU 加速** — 整个路径追踪管线（光线生成、求交、着色、累积）均在 GPU 上运行。当 CUDA 不可用时，回退到 CPU 多线程渲染。
...
| 路径追踪（5次反弹） | CPU 标量循环 | GPU SIMT 并行 |
| 随机数生成 | PCG 哈希（CPU） | PCG 哈希（GPU `__device__`） |
| 累积缓冲 | 主机 `glm::vec4[]` | 设备 `float4[]` |
| 显示 | Walnut::Image（Vulkan） | Walnut::Image（Vulkan）通过 D2H 拷贝 |
| 俄罗斯轮盘赌 | CPU | GPU |

**GPU 内核布局**：16×16 线程块，每个像素对应一个 CUDA 线程。`RenderKernel` 对每个像素执行完整的路径追踪，包括光线-球体求交、俄罗斯轮盘赌终止、朗伯漫反射 BRDF 采样以及渐进累积。

## 要求

- **NVIDIA GPU**（可选）计算能力 ≥ 7.5（Turing / Ampere / Ada / Blackwell）
...
- **Vulkan SDK 1.4+**
- **Visual Studio 2026**（或 2022，向下兼容）需支持 C++20
...
- **[Vulkan SDK](https://vulkan.lunarg.com/)** — 显示所需。请安装到默认位置。
...
### 4. 构建并运行
在 Visual Studio 2026 中打开 `RayTracing.slnx` 并构建（推荐使用 Release 或 Dist 模式以获得最佳性能）。
...
│   ├── Renderer.h/cpp         # 渲染器类（CPU/GPU 分派）
...
GitHub Actions 在每次推送和拉取请求时执行构建：
- **Windows Server 2022** 搭载 CUDA 12.8 + Vulkan SDK
- Debug 和 Release 配置
- Release 构建提供可下载的构建工件
...
- 说明\
这是一个简单的 Walnut 应用模板——与 Walnut 仓库中的示例不同，此模板将 Walnut 作为外部子模块，更适合实际应用程序开发。更多详情请参阅 Walnut 仓库。
- 入门指南\
克隆后，您可以根据需要自定义 `premake5.lua` 和 `WalnutApp/premake5.lua` 文件（例如将名称从 "WalnutApp" 改为其他名称）。满意后，运行 `scripts/Setup.bat` 生成 Visual Studio 2022 解决方案/项目文件。您的应用程序位于 `WalnutApp/` 目录中，其中 `WalnutApp/src/WalnutApp.cpp` 包含一些基本的示例代码供您入门。我建议修改该 WalnutApp 项目来创建您自己的应用程序，因为一切已设置就绪。
